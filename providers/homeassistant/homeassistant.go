...
func (p *Config) Open(ctx context.Context) error {
	...
	finalResponse, err := p.haGetEntityState(ctx, p.SwitchEntityID)
	if err != nil {
		return fmt.Errorf("failed to get Home Assistant entity state: %w", err)
	}

	p.Logger.Info("Home Assistant entity state", "entity", p.SwitchEntityID, "finalResponse", finalResponse)
...
func (p *Config) PowerStateGet(ctx context.Context) (state string, err error) {
	...
	p.Logger.Info("Home Assistant PowerStateGet", "entity", p.SwitchEntityID, "finalResponse", finalResponse)
